...
thm lookup_trie.induct
term "map_of"
value "map_of ([]::(nat\<times>nat) list)"
value "(map_of [(1::nat,2::nat),(3,4),(5,6)] 1)"
...
"update_trie ks v = update_with_trie ks (\<lambda>_. v)"
...
value"invar_trie (Trie None ([(1,Trie None [])]::(int \<times> (int, int) trie) list))"
...
lemma invar_trie_update: "invar_trie t \<Longrightarrow> invar_trie (update_trie ks v t)"
  apply(induct ks t rule: update_trie_induct)
   apply auto
  apply (auto simp:set_update_with_aux)
   apply (auto split: option.splits)
  apply(auto simp: update_not_empty)
  done
